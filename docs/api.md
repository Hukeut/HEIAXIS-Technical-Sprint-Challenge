...
- C. A plain HTML front page at `GET /` showing only the HEIAXIS name, with the fuller terminal-style summary and office caseload detail moved to their own dedicated HTML pages, everything else stays JSON with self-explanatory envelopes.

**Chosen Answer:** Option A was the original approach, and it works, but it quietly reintroduces the exact problem this whole project is built to avoid: a caller sees a `confidence: "Medium"` or a `gap_type: "unowned_handoff"` with no explanation attached unless they already have `docs/data_dictionary.md` open next to them. Option B fixed that, but tried to solve it everywhere including the front page, and a JSON directory-of-endpoints as the very first thing someone sees is exactly the kind of "too complicated" first impression this was supposed to avoid, useful to a program, not to a person glancing at a browser. An earlier version of this design put the full terminal report directly on `GET /`, which turned out to have the same problem one level down, still too much for a first glance, and the version after that stripped it down to just the name, which was clean but left a first-time visitor with no way to find anything short of already knowing the URLs. C settles between those two: the front page carries a short, plain-language explanation of what the project actually does, plus a button and a one-line explanation for every other route, so a person can find their way around without reading `curl` commands in a README.

**Low Level Explanation:** `GET /` returns `text/html`: the HEIAXIS heading, a short paragraph explaining the project (students, offices, the two kinds of signal), and a button linking to every other route with a one-line description next to it, replacing the `curl` walkthrough that used to live only in the README. `GET /report` returns the fuller `text/html` page: the same report `pipeline.py` prints to the terminal (data-quality report, both outputs' previews, office caseload) in a `<pre>` block under the same heading, rebuilt from whatever is currently in `output/` rather than by re-running the pipeline. `GET /office-caseload/print` and `GET /office-report` are the same style of page, scoped to just the office caseload table (plus explanation) and the caseload-vs-gaps comparison, respectively. Every other endpoint returns `application/json`. List endpoints (`/students/flagged`, `/continuity-gaps`, `/office-caseload`) return an envelope with `description`, `fields` (a plain-English definition of every column), `count`, `filters_applied`, and `results`. Where relevant, the envelope also includes `confidence_scale` and, for continuity gaps, `gap_types`. The single-student, summary, and data-quality-report endpoints follow the same explanatory pattern at a smaller scale.
...
|---|---|---|
| `GET /` | HTML front page: a short project explanation plus a button and one-line description for every other route below. | none |
| `GET /report` | HTML page with the full plain-text summary `pipeline.py` prints to the terminal (data-quality report, both outputs' previews, office caseload). | none |
...
| `GET /office-caseload/print` | The office caseload table as its own small HTML page in the same style as `/report`, followed by the same explanation of what the numbers mean and what they suggest doing next. | none |
| `GET /office-report` | One plain HTML table pairing each office's cases-per-staff ratio with how many continuity gaps actually trace back to it, sorted by ratio, followed by a short explanation of exactly how that gap count is measured. A gap naming more than one office counts once for each office named. | none |
